...
ExitBathing = function(animscene, town, cam)
	...
	TriggerServerEvent("rsg-bath:setBathAsFree", town)
...
EnterPremiumBath = function(animscene, town, cam)
	--TriggerServerCallback("rsg-bath:canBuyDeluxeBath", function(cb) // link own callback system
		--if cb then 
			...
			TriggerEvent("rsg-bath:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@ARTHUR`, `DEFAULT`, "BATHING" })
			TriggerEvent("rsg-bath:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@MAID`, `DEFAULT`, "BATHING" })	
...
Action = function(name, p1, p2, p3)
	...
	if (name == "START_BATHING") then
		--TriggerServerCallback("rsg-bath:canEnterBath", function(cb) if cb then EnterBathing(p1) end end, p1) // link own callback system
